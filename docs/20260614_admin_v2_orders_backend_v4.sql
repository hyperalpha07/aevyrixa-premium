-- Phase 2.2 Admin V2 Orders Backend Completion - security-reviewed v4
...
-- Do not run this file until the Admin V2 server code is ready for the v4 RPC
...
begin
  ...
  select exists (
    ...
    group by c.oid
    -- pg_attribute.attname is type name; cast to text so array_agg and the literal text[] compare without name[]/text[] operator errors.
    having array_agg(a.attname::text order by ck.ord) = array['order_ref']::text[]
  ...
  loop
    if not exists (
      ...
      raise exception 'Preflight failed: existing table % has no primary key. Review the previous partial migration before running v4.', v_table;
    end if;
  end loop;
  ...
  if to_regclass('public.order_notes') is not null then
    ...
    loop
      if not exists (
        ...
        raise exception 'Preflight failed: existing public.order_notes.% is missing or not %. Review the previous partial migration before running v4.', v_col.attname, v_col.expected_type;
      end if;
    end loop;
    ...
    if not exists (
      ...
      raise exception 'Preflight failed: existing public.order_notes.order_ref does not reference public.orders(order_ref). Review the previous partial migration before running v4.';
    end if;
  end if;
  ...
  if to_regclass('public.order_events') is not null then
    ...
    loop
      if not exists (
        ...
        raise exception 'Preflight failed: existing public.order_events.% is missing or not %. Review the previous partial migration before running v4.', v_col.attname, v_col.expected_type;
      end if;
    end loop;
    ...
    if not exists (
      ...
      raise exception 'Preflight failed: existing public.order_events.order_ref does not reference public.orders(order_ref). Review the previous partial migration before running v4.';
    end if;
  end if;
  ...
  if to_regclass('public.invoices') is not null then
    ...
    loop
      if not exists (
        ...
        raise exception 'Preflight failed: existing public.invoices.% is missing or not %. Review the previous partial migration before running v4.', v_col.attname, v_col.expected_type;
      end if;
    end loop;
    ...
    if exists (
      ...
      raise exception 'Preflight failed: existing public.invoices.actor_source is not text. Review the previous partial migration before running v4.';
    end if;
    ...
    if not exists (
      ...
      raise exception 'Preflight failed: existing public.invoices.order_ref does not reference public.orders(order_ref). Review the previous partial migration before running v4.';
    end if;
    ...
    if not exists (
      ...
      group by c.oid
      -- pg_attribute.attname is type name; cast to text so array_agg and the literal text[] compare without name[]/text[] operator errors.
      having array_agg(a.attname::text order by ck.ord) = array['invoice_number']::text[]
    ) then
      raise exception 'Preflight failed: existing public.invoices.invoice_number is not unique. Review the previous partial migration before running v4.';
    end if;
  end if;
end $$;
